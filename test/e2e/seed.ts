...
// independently of the app's edge client.
//
// `pgbouncer=true` disables prepared statements. The local Postgres is reached
// through a connection pooler that reuses sessions, so Prisma's cached
// statements collide with `prepared statement "s0" already exists`.
...
function mintToken(userId: string): string {
  return jwt.sign({ userId }, process.env.JWT_SECRET || 'e2e-test-secret', {
    expiresIn: '1h',
  });
}
