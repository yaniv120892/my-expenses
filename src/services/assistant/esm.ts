...
/** What mastra.mjs re-exports, typed from the packages themselves. */
export interface MastraExports {
  Agent: typeof import('@mastra/core/agent').Agent;
  createTool: typeof import('@mastra/core/tools').createTool;
  RequestContext: typeof import('@mastra/core/request-context').RequestContext;
  Memory: typeof import('@mastra/memory').Memory;
  PostgresStore: typeof import('@mastra/pg').PostgresStore;
}

export function loadMastra(): Promise<MastraExports> {
  ...
  const entry = pathToFileURL(path.join(__dirname, 'mastra.mjs')).href;

  return dynamicImport(entry) as Promise<MastraExports>;
}
